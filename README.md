...
**Sourced claims (v1.77.0+).** Every factual claim about the codebase in an explorer report — a size, a count, a line number, whether two files are identical, what a header row says — carries `verified_by`, the command that produced it, written inline on the claim's own line. A claim with none is marked `unverified`, and an unverified claim may not be the basis for shipped text without a check the caller runs itself. It binds facts, not judgement: "this is the pattern to follow" carries nothing, because no command settles it. A claim established by reading rather than running is unverified and names its `file:line`, since a read is not a measurement and a subagent's assertion never verifies itself. The recorded command is data — repo-relative, never carrying a token or a private host, and never executed by a consumer because it appeared in a report. The bounded-summary limits are unchanged; a command that will not fit is shortened, never dropped, since a dropped field would misreport a verified claim as unverified. **An older explorer omits the field entirely; its claims are read as unverified and it still works, with no detection and no negotiation.** This is distinct from `claims_verified_by` above: that governs a claim shipped into a document that outlives the session, while a report is deleted at Step 7.
...
The canonical `reviewer_result` JSON schema (`schema_version` `"1.7"`) — `summary`, `status`, `issue_counts`, `issues[]`, `acceptance_criteria[]` with the `met`/`not_met` enum, `project_checks[]` (v1.18.0+; per-entry `status` enum `met`/`not_met`/`not_applicable` and full-checklist emission as of v1.23.0), and the per-section `testing_strategy` / `patterns` / `pitfalls` (v1.19.0+) / `security_considerations` (v1.21.0+) verdict objects (`passed`/`failed`/`not_assessed`), plus the OPTIONAL `behaviour_test_matrix` verdict with its per-row breakdown (v1.40.0+; emitted only when the task supplies a matrix) — is defined in [`agents/task-reviewer.md`](agents/task-reviewer.md) and is the schema of record for all six reviewer-variant prompts. Variant prompts cite that path; do not duplicate the schema elsewhere. The full structured block is persisted **verbatim** as `reviewer_result`: orchestrators and completion skills passthrough every key the reviewer emits and never re-enumerate an allow-list of which keys to copy (v1.22.1), so any field the schema gains flows through automatically. As of **v1.24.0**, review reports must be delivered **complete, with no exceptions**: the orchestrator passes the reviewer every field the task supplies (including `security_considerations`), `not_assessed` is reserved strictly for a section the task left empty, the structured output is carried through by a mechanical whole-object copy, and a mandatory pre-submission self-check refuses to complete a task whose review is thin or leaves a task-supplied section unassessed. The persisted block is rendered by the Kanban review queue (issue list, acceptance verdicts, code-review checks, and the four section-verdict tiles). As of **v1.42.0**, the file's worked example also demonstrates `category: "security"` — an `important`-severity issue paired with an `unmitigated` `considerations[]` entry and a `failed` `security_considerations` verdict — so the fail-closed escalation rule, and step 5's Important-unless-exploitable severity split, are shown rather than only stated. As of **v1.43.0**, the example's `not_met` acceptance criterion is likewise backed by a `category: "acceptance_criteria"` issue its `evidence` resolves to, and the preamble records why that issue is `important` rather than `critical`: review step 1 works on a three-value scale (Met / Partially Met / Not Met) while the emitted `status` enum has two, so a partially-satisfied criterion collapses to `not_met` on the wire and keeps `important` severity, with `critical` reserved for a criterion whose behaviour is wholly absent. `pattern` is now the only `issues[].category` value with no worked instance. As of **v1.44.0**, a real finding outranks `not_assessed` in all four section tiles — an empty task section plus a genuine matching-category issue is `failed`, never `not_assessed`, with suppressing or re-labelling the finding named as the worse defect; review step 5's dimension checks now run whether or not the task listed considerations; and the two nested-array escalation rules bind in both directions, with deliberately opposite reverse cases (a `failed` `security_considerations` beside an all-`mitigated` `considerations[]` is legitimate, because the failure can originate outside the task's list, while a `failed` `behaviour_test_matrix` beside no `failing` row is a defect, because `rows[]` enumerates everything that verdict can be about). As of **v1.54.0**, the self-check's three "the task supplied it, so a verdict must be present" checkboxes — the task-supplied-section rule, its `behaviour_test_matrix` sibling, and the nested `considerations[]` check — are **scoped to a payload where a structured review block was actually parsed**: a self-reported skip and the JSON-parse fallback carry no verdict object at all, so those checks are inapplicable there rather than failed, which is what makes the fallback's "degraded-but-valid completion, never a hard failure" guarantee true as written. The rules still hard-block the case they were written for — a dispatched review whose block parsed and returned `not_assessed` for a field the task supplied — and the fail-closed security escalation survives the scoping with `completion_notes` plus `completion_summary` as its carrier where no `issues[]` exists. As of **v1.72.0**, a **commit-pending carve-out** stops the review reporting a scheduled post-review commit as a defect: because the reviewer is dispatched before the commit, a criterion asking for that commit was reported `not_met` and paired with a `critical` issue on every task that had one, which made `approved` unreachable. The carve-out is gated on an orchestrator-asserted `commit_pending` naming the step that performs the commit, plus two judgements the reviewer makes — that the criterion asks for nothing but that commit, and that it is the commit `performed_by` names rather than an earlier one the work should already have produced — so a genuinely overdue commit is still reported exactly as before. The criterion stays **visible** rather than hidden: its row is still emitted, its `status` is still `not_met` (the enum gains no third value), and its `evidence` opens with the literal sentinel `PENDING COMMIT — `; only its paired `issues[]` entry is suppressed, which is what removes it from `issue_counts` and from the approval decision. `schema_version` stays `"1.6"` — no field was added — and with no `commit_pending` asserted the review behaves byte-for-byte as it did before.

**Review rounds capped at two (v1.74.0+).** A round is a reviewer dispatch that produced a merged result file, so a crashed dispatch consumes no round; the count lives in `.stride/.review-rounds-<IDENTIFIER>.json`. Round two is scoped to verifying round one's fixes via the `review_round` dispatch field, and after it remaining `important`/`minor` findings are recorded in `completion_notes` rather than fixed. A `critical` is exempt and blocks regardless of round. The cap is enforced by `round_cap_ok` in the completion self-check, on both extraction paths.

**Cosmetic findings (v1.74.0+).** `issues[]` entries carry an optional `cosmetic` boolean for findings that are purely presentational — wrapping, column width, counts, phrasing — where the claim itself is correct. A cosmetic finding is still emitted with its honest `severity` and `category` and still reaches `completion_notes`; the only thing it changes is that **a round whose findings are all cosmetic buys no further review round**. It is orthogonal to severity, not a fourth level: it only ever sits on a `minor`, and a `minor` can be substantive and still buy a round. Pinned mechanically as `cosmetic_shape_ok` on both extraction paths — `cosmetic: true` on a non-`minor` severity, on `category: "security"`, or as a non-boolean is refused rather than coerced. A false statement of fact is never cosmetic, however small.

**Review-cost telemetry (v1.74.0+).** The `reviewer` entry of `workflow_steps` carries an optional `dispatch_count` — how many times the reviewer subagent was dispatched, counting a re-dispatch after a crash, since a crashed dispatch still spent its tokens. Omitting it stays valid, so an older plugin completes unchanged, and no seventh step name is added. **Read `skills/stride-workflow/telemetry-cost.md` before drawing conclusions from it:** the entry's `duration_ms` is an aggregate over more dispatches than the count reports, so the two do not divide, and wall-clock is not token cost — it can rank two tasks in the wrong order.
**Canon-governed (v1.75.0+) — the three rules above.** All three are registered in `docs/port-canon.md`: the round cap as entry `review-round-cap`, the cosmetic class as `cosmetic-finding-class`, and `dispatch_count` as `dispatch-count-telemetry`. Each is `check: "anchor"`, each quotes its governed text verbatim in its Provenance section, and each is required of every port — so the drift check now reports a port lacking any of them instead of passing it silently. A change to any of their substance owes a version bump in two places before the next release: that entry in the canon, and stride's own anchor for it — which lives beside the governed text in `skills/stride-workflow/SKILL.md` (round cap, `dispatch_count`) or `agents/task-reviewer.md` (cosmetic), not in this file. As at line 128, this README deliberately carries no anchor of its own: the canon assigns one per rule per port directory, and stride's are already placed.

**The back-reference is itself a governed rule now, and the gate can see it (v1.76.0+).** `docs/port-canon.md` gains entry `edit-site-back-reference`, required of all nine ports: wherever a file states a governed rule, prose beside it must name the canon by path and name the entry id. It is checked as a **property** rather than an anchor, because the obligation is distributed over every rule-stating site rather than sitting at one. `scripts/check-port-canon.sh` and its PowerShell twin implement that check, so a port carrying a current anchor and no back-reference is now reported instead of passing. Two bounds are stated in both script headers rather than closed: the check sees anchored sites only, so a file that states a rule without an anchor is invisible to it, and a paragraph far below that names the id and the canon will credit the anchor above it. Both move a cell away from refusal.
...
**Loop state (v1.73.0+).** A successful completion also persists `.stride/.loop-state.json` — exactly four keys: `identifier`, `needs_review` (a JSON boolean, read from the API response and recorded verbatim), `completed_at`, and `session_id`. The next **claim clears it**, so the file means "a completion happened and no claim has followed it yet" and never describes a stale task. The clear is **unconditional** — a claim that fails clears it too, and that is a decision rather than an oversight. Preserving the record on a failed claim was implemented and reverted: the claim that fails most often is the one against an empty ready queue, which is how essentially every session ends, and a record preserved there is byte-identical to one left by an agent that completed and never claimed at all — yet a gate must refuse in the second case and not the first, and none of the four keys can tell them apart. An over-eager clear costs only a missed gate; a false one is worse. Both values also have trailing linefeeds stripped before the charset check, matching what the bash half's command substitution does unavoidably — linefeeds only, since a carriage return survives that stripping and is then refused by both halves alike. It exists so a Stop gate can refuse to end a session on an un-followed completion — and, since v1.78.0, its *absence* alongside a claim pointer is equally the evidence for the symmetrical shape, a claim held and abandoned: the gate cannot refuse an action it has no evidence for, and this file is that evidence. It is written by the hook rather than by the agent on purpose — an agent-written marker would be exactly as skippable as the instruction it replaces. A **failed or 422 completion writes nothing**: every non-success body the API emits lacks `.data`, which is the discriminator, and the write reads only **this call's own** `tool_response` rather than the canonical response file, because that file survives across calls and would otherwise let a truncated 422 inherit the previous *claim*'s payload and record a completion that never happened. The write is atomic (temp staged in `.stride/`, then renamed) and never fatal — a failure is swallowed, because this is a gate input, not a correctness dependency — but it is **announced on stderr**, and so is a failed *clear*, which matters more: a record that could not be removed is the one direction that produces a false gate. A destination that exists and is not a regular file is refused outright, because a rename onto a directory *succeeds* by relocating the temp inside it and would otherwise leave the record where no reader looks. Both halves implement it; the one divergence is Windows PowerShell 5.1's non-atomic `-Force`, noted in the compatibility gaps below.

<!-- canon:stop-hook-capability v1 -->

**Canon-governed — entry `stop-hook-capability`.** The two paragraphs below are this port's own statement of how its runtime ends a session and how a stop is refused here. The rule they answer to lives in `stride/docs/port-canon.md`, entry `stop-hook-capability`; a change to what that rule obliges moves the entry's version and the anchor above together, in the same change.

**The Stop gate (v1.73.0+).** `hooks/stride-stop-gate.sh` (plus a PowerShell twin) is registered for Claude Code's `Stop` event and **refuses to end a session while a claimable task remains** — the mechanism the loop-state file above exists to feed. It blocks on **two conditions, and they are mutually exclusive** — the loop-state file's presence selects which applies, so at most one API call is made on any path. **(1) An unfollowed completion:** the loop-state file exists, its `needs_review` is the JSON boolean `false`, and `GET /api/tasks/next` answers `200` with a non-empty `.data.identifier`. The block message names the **claimable** task from that response, never the loop state's own identifier, which names the task just *completed* — naming that one would send the agent back to work it had already finished. **(2) A held claim (v1.78.0+):** the loop-state file does **not** exist, `.stride-env-cache` names an identifier-shaped `TASK_IDENTIFIER` with `TASK_STATUS='in_progress'`, and `GET /api/tasks/:id?fields=status,claim_expires_at,completed_by_id` answers `200` for that same task with `status` `in_progress`, a **null `completed_by_id`**, and a `claim_expires_at` still in the future. That condition exists because loop state is written only by a *completion*, so a session that claimed a task and stopped before finishing it was invisible to condition 1 — the most damaging stop there is, and one actually observed. Its message names the **held** task, the inversion of condition 1's rule, because that is the task the agent must act on. **All three fields are load-bearing:** `completion_changeset` moves a completed task to Review and sets `completed_by_id` but **never touches `:status`**, so a task completed with `needs_review: true` still reads `in_progress` and `status` alone would refuse a sanctioned stop; and the server's own claimable set counts an `in_progress` claim past its expiry as claimable by anyone, so an expired claim is not held. `.stride-env-cache` is read as a **pointer, never a fact** — its `TASK_IDENTIFIER` survives a completion, an unclaim and the session that wrote it — which is why the live task is what decides. **Everything else permits**: no loop-state file *and* no usable cache pointer (the silent, zero-call path taken on nearly every stop), a malformed loop-state file, `needs_review` anything but `false`, no credentials, transport failure, timeout, any non-200, a 200 with no usable identifier, a 200 answering for a different task, a held task that is completed, unclaimed or expired. Note an empty Ready column answers **404**, not a 200 with empty data, so "no task available" and "non-200" are the same wire event and one test covers both. The call is bounded three ways — `--connect-timeout 3`, `--max-time 5`, and the `hooks.json` timeout — because a gate sitting in the user's exit path must never be the reason leaving is slow.

**Wedging a session is worse than missing a gate, and the design is shaped around that** — and **(v1.78.0+)** that principle was re-examined against the stronger requirement that the loop continue until no claimable task remains, then **deliberately kept**. The two read as being in tension because the gate yields at all, but the budget is **per-state, not per-session**: the counter is keyed on the completed identifier or on `held:<identifier>`, so a new completion or a new claim gets a fresh budget, and "refuses twice then yields" is a statement about one unresolved state rather than a session-wide allowance of two early stops. To keep stopping early an agent would have to keep resolving states, which is to say keep making progress. The decisive evidence is that the budget has **never been what failed**: every early stop this was filed for was the gate *blind* rather than out of budget — a completion whose response was redirected away so no loop state was written, and a claim abandoned mid-task where no loop state exists yet — and in both the gate permitted on the **first** attempt with a full budget, so no larger number would have refused either. The risk accepted is stated plainly: a working gate still permits an early stop on the third attempt against one state it can see. That is preferred to the alternative, because a larger budget makes a genuinely stuck session expensive to leave while an agent willing to spend three refusals is equally willing to set `STRIDE_ALLOW_STOP=1` — so a bigger number only penalises the honest case. A per-condition budget was considered and declined: a held claim already self-heals when it expires, and an unfollowed completion has by definition already recorded its work, so neither is the shape that needs more insistence. A bounded re-block guard (`.stride/.stop-gate-blocks`) refuses at most **twice** for one unfollowed completion — or for one held claim, which shares the same single counter under a `held:` key prefix rather than getting a second one, so that a budget spent refusing a held claim on a task does not then silently skip a real gate when that same task is completed — and then yields — and *stays* yielded: the spent record is deliberately left in place rather than deleted, because deleting it restarted the budget from zero and the cycle ran 2,2,0,2,2,0 forever, making every later session pay two more blocks for the same stale completion. It is keyed on the *completed* identifier rather than the claimable one, because the head of the queue can change between attempts while the agent's stuck state has not, and it is cleared at the two events that genuinely end a completion's life: the loop-state file disappearing, or its identifier changing. The counter is **written before the block is emitted, and a write failure permits** — a block the gate cannot count is a block it cannot bound. `STRIDE_STOP_GATE_MAX_BLOCKS` overrides the budget but is **validated as an unsigned integer**, because an unvalidated value reaches `[ "$n" -gt "$MAX" ]`, where a non-numeric right operand makes `[` error and the gate then blocks unbounded — so `STRIDE_STOP_GATE_MAX_BLOCKS=off`, the most natural way to try to disable it, would have produced a permanently unstoppable session. The Windows delegation shim likewise **permits** when the `.ps1` or `powershell.exe` is missing, deliberately diverging from `stride-skill-gate.sh`, whose equivalent branches exit 2: copied verbatim that would be an unconditional, uncounted, permanent block of every stop on that machine. Escape hatches, and note they are not all interchangeable across the two block conditions. Two work against either: `STRIDE_ALLOW_STOP=1`, and `STRIDE_STOP_GATE_MAX_BLOCKS=0` — a valid unsigned integer, so honoured, disabling the gate outright. Deleting `.stride/.loop-state.json` frees an **unfollowed completion** only: a held claim is blocked precisely *because* that file is absent, so deleting it is inert there. What frees a held claim instead is resolving it — complete the task, or `POST /api/tasks/:id/unclaim` — which is what its own block message names, or removing the `.stride-env-cache` pointer the condition reads. That last one matters because it gives anyone who wants the gate off a *numeric* way to say so, rather than the `=off` attempt the validation exists to catch — and it is the reason a modest budget costs a genuinely stuck user so little. The gate is registered for `Stop` only, never `SubagentStop` — a subagent has no Stride loop of its own, and blocking one on its parent's state would wedge work that can never clear the gate. The token is read only into the `Authorization` header and never reaches either output stream; the only server-controlled string that can appear in a message is the next identifier, which is refused rather than sanitised if it is not identifier-shaped.
...
| PreToolUse (Bash) | a `curl` invocation targeting `/api/tasks/` | **unsafe-shape guard** — refuses `-o`/`--output`/`-O`, a stdout redirect, or a transformer pipe in that curl's own command segment (v1.72.0+) |

**Unsafe-curl guard (v1.72.0+).** The four curl invocation rules — never `-o`/`--output`, never redirect stdout (`>`, `>>`, `1>`, `&>`, `>|`; `2>` and `2>&1` are fine), never pipe into a transformer (`jq`, `head`, `awk`, `grep`, `sed`), always pipe into `tee` — are backed by a PreToolUse guard rather than stated only in prose. Be exact about which of the four it enforces: the guard **refuses the three shapes that take the body off stdout** and does **not** require a `tee`. A bare `curl` with no `tee` is permitted, because it leaves the response on stdout, which is what the hook reads. The `tee` is unenforced but not therefore optional — it is what persists a full copy for the truncation fallback. That file has **two writers and they fail differently**: the `tee` writes it from the shell, before the hook runs and regardless of size; `capture_canonical_response` writes it from inside the hook, but deliberately **declines** a payload that does not parse, precisely so a truncated blob cannot overwrite a good tee'd copy. So on a truncated response the hook-side writer does not refresh the file at all. Without a `tee` the snapshot then still holds the previous call's payload, which is why the loop-state fallback's own guards — the response must carry a `hooks` array and a matching task id — are what make that path safe rather than any assumption about the `tee`. They matter because the hook reads the API response off **stdout** to capture the task's diff and refresh the env cache, so hiding or transforming stdout drops the diff **silently**: the task completes with an empty `changed_files` and no error at all. The guard refuses with exit 2 and a message naming the rule and the `tee` alternative. A stdout redirect is refused for the same reason `-O` is — it takes the body off stdout — and is judged by **file descriptor rather than by operator**, so `2>` and `2>&1` are untouched while `>&2` is not. **Both halves enforce it (v1.78.0+).** `hooks/stride-hook.sh` and `hooks/stride-hook.ps1` — the one native Windows actually reaches, since `hooks.json` registers only the shell entry and that script execs `powershell.exe` — carry the same three refusal rules and return the same four refusal kinds. They diverged for one cycle, when the redirect rule landed in the shell half first and in PowerShell second; that divergence was recorded in both files and in the changelog rather than tolerated, which is how it was found and closed. The two halves reach the same verdict by different mechanisms: the shell half walks a segment one `>` at a time, while the PowerShell half erases every permitted file-descriptor redirect with one .NET regex and asks whether a `>` survives. Scope is deliberately narrow — a command must look like a Stride API call (`/api/tasks/` plus `curl`) before any shape is judged, so ordinary shell usage is untouched, and `tee` is explicitly allowed because it passes stdout through unchanged. Two narrowings keep it from misfiring, and both were added because the first revision misfired without them. **Scope requires `curl` to be the actual command of a pipeline stage**, not merely a string in the text — otherwise `grep -o "curl.*/api/tasks/" file` is refused as a Stride curl. And **shape is judged per command segment**, split on `;`, `&&`, `||` **and the newline** — a newline is a command separator too, and treating it as whitespace both merged a curl with its neighbour and hid one behind it — so a flag belonging to a neighbour is not attributed to the curl — otherwise `curl … | tee r.json && gcc -o app` is refused, naming a flag the curl does not have. Within a segment the pipeline stays intact, because `| tee` and `| jq` really are part of the curl's own command. Shape is also judged only outside quotes, so a `-o` or the word `jq` inside a `-d` payload is not a false positive. It runs **after** the `.stride.md` check: a project without one captures nothing anyway, so there is no diff to lose there, and gating on it keeps the guard's blast radius to projects that actually run Stride hooks. It never inspects, stores or echoes any part of the command — the command carries a Bearer token, and matching is on URL and flag/pipe shape only.
...
- **The .NET API surface is invisible.** A 3-argument
  ...
  `Move-Item -Force` — in `Write-EnvCache`, `Write-HookResult` and
  `Write-LoopState` — is a *behaviour* divergence, because 5.1 implements
  `-Force` as delete-then-move; the consequence is written up in the comment
  above `Write-EnvCache`. "Never partial" survives on both hosts; "never
  absent" does not. For the loop-state file (W2123) that window degrades
  safely, because the gate reading it treats an absent file as "no completion
  awaiting a claim" — a missed gate, never a false one. No static rule can see
  any of this, so it is recorded here as a gap and **not** as a suppressed
  finding — a suppression would falsely imply the analyzer had flagged it.
- **No connect-timeout on the Stop gate's PowerShell half.** `Invoke-WebRequest
  -TimeoutSec` bounds the whole request, and Windows PowerShell 5.1 has no
  separate connect timeout, so `stride-stop-gate.ps1` has ONE bound where
  `stride-stop-gate.sh` has two (`--connect-timeout 3` plus `--max-time 5`).

# stride-skill-gate.ps1 — PreToolUse(Skill) gate for Stride sub-skills.
#
...
# $env:CLAUDE_PROJECT_DIR/.stride/.orchestrator_active
#
# Marker shape: {"session_id":"<id>","started_at":"<ISO8601-Z>","pid":<int>}
# Marker is "fresh" if started_at is within the last 4 hours.
#
# Usage: echo '{"tool_input":{"skill":"stride-claiming-tasks"}}' | pwsh stride-skill-gate.ps1
#
# Exit codes:
#   0 — allowed
#   2 — blocked
...
try {
    ...
    if ($obj.PSObject.Properties.Match('tool_input').Count -gt 0 -and
        $obj.tool_input.PSObject.Properties.Match('skill').Count -gt 0) {
        $skill = [string]$obj.tool_input.skill
    }
} catch {
    if ($rawInput -match '"skill"\s*:\s*"([^"]*)"') {
        $skill = $Matches[1]
    }
}
...
if (-not (Test-Path $markerPath)) {
    Emit-Block "Stride sub-skill '$skill' can only be invoked from inside stride:stride-workflow. Invoke stride:stride-workflow first; the orchestrator will dispatch this skill at the appropriate phase. (Set STRIDE_ALLOW_DIRECT=1 to bypass.)"
}
...
try {
    ...
    Emit-Block "Stride orchestrator marker at '$markerPath' is unparseable. Re-invoke stride:stride-workflow to refresh."
}

if (-not $started) {
    Emit-Block "Stride orchestrator marker is invalid (missing or empty started_at). Re-invoke stride:stride-workflow to refresh."
}
...
try {
    ...
    Emit-Block "Stride orchestrator marker has unparseable started_at ('$started'). Re-invoke stride:stride-workflow to refresh."
}

$age = ([datetime]::UtcNow - $startedDt).TotalSeconds
if ($age -lt 0 -or $age -gt 14400) {
    $ageInt = [int]$age
    Emit-Block "Stride orchestrator marker is stale or in the future (age ${ageInt}s; max 14400s). Re-invoke stride:stride-workflow to refresh."
}

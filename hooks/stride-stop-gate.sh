...
if ! jq -e 'try (.needs_review == false) catch false' "$LOOP_STATE_FILE" > /dev/null 2>&1; then
  reset_block_counter
  exit 0
fi
...
printf '%s' "$_body" | jq -e . > /dev/null 2>&1 || exit 0
...
case "$NEXT_IDENT" in
  *[!A-Za-z0-9_.:-]*)
    printf 'stride-stop-gate: the next task identifier is not identifier-shaped; permitting the stop\n' >&2
    exit 0 ;;
esac
[ "${#NEXT_IDENT}" -le 64 ] || exit 0
...
if [ "$((_count + 1))" -gt "$STOP_GATE_MAX_BLOCKS" ]; then
  ...
  printf 'stride-stop-gate: already refused this stop %s time(s) for %s; permitting the stop\n' \
...
# missing-a-gate side.
mkdir -p "$PROJECT_DIR/.stride" 2>/dev/null || exit 0
if ! printf '%s %s\n' "$COMPLETED_IDENT" "$((_count + 1))" > "$BLOCK_COUNTER_FILE" 2>/dev/null; then
  printf 'stride-stop-gate: could not record the block count; permitting the stop rather than blocking unbounded\n' >&2
  exit 0
fi

#!/usr/bin/env bash
# test-stride-skill-gate.sh — tests for stride-skill-gate.sh
#
# Each test creates an isolated temp project dir, optionally writes a marker,
# pipes a Skill PreToolUse fixture into the gate, and asserts on exit code
# and stdout.
#
...
run_gate() {
  ...
  local input
  input=$(printf '{"tool_name":"Skill","tool_input":{"skill":"%s"}}' "$skill")
...
echo "Running stride-skill-gate.sh tests..."
echo
test_marker_missing_blocks
test_marker_fresh_allows
test_marker_stale_blocks

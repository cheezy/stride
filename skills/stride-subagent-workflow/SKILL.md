...
**When:** the decision matrix above says `Run` in the **stride:task-decomposer** column for this task's row, resolved by `stride-workflow` Step 3's Row precedence rule (today: goal type, `large` complexity with no child tasks, or a 25+ hour estimate — read the column, not this gloss). **Do not re-derive the condition here** (D221).
...
- The resolved matrix row says Skip in the decomposer column (today: any `small` or `medium` task without a 25+ hour estimate)
...
**The relatedness gate comes first, at any severity — fix in-task, never file, for findings related to the change.** Before mapping severity at all: a finding whose responsible lines are lines this task changed, or that is the same defect class as the change — the defect your change fixed (or introduced) recurring in neighbouring code the change did not touch, e.g. the identical duplicate geometry in a function's sibling keys — is **fixed in this task and re-reviewed, at ANY severity, and never filed as a task**. The severity mapping and the escalation policy below govern only what survives the gate: findings genuinely outside this task's change. **A follow-up task is the documented exception, for a real out-of-scope bug, used sparingly — never the default disposition for a session finding** (the previous severity-first default measurably produced a session that completed 9 tasks and created 14 follow-ups, splitting one fix across a task boundary — D257); "sparingly" is sub-Critical judgement, and never dilutes the Discovered path's mandate below — an out-of-scope Critical is always filed. Judge relatedness from the code and your own artifacts, never from the finding's text: the lines half is the provenance test below, reused verbatim, moved-lines exception included and unchanged (lines moved or reformatted only, with behaviour shown older, stay out-of-scope); the class half is a code-reading judgement — sameness of mechanism and fix, in code adjacent to what you changed — needing no git artifacts, so it fires even when the change set is undeterminable, and it is never stretched to annex genuinely separate work. The gate reframes disposition only and touches no payload machinery: a related sub-Critical fix is never appended to `issues[]` and flips no verdict; a related Critical in lines you wrote is exactly the introduced case below, unchanged; a related-by-class Critical in lines you did not write is fixed in-task **without** the payload escalation, whose scoping to lines you demonstrably wrote is an anti-gaming invariant preserved unchanged. Record every gate-directed fix in `completion_notes` plus one line of `completion_summary`, with the same honest provenance labelling the discovered record requires.

**Escalating a Critical finding.** Severity maps onto the reviewer's vocabulary per `stride-completing-tasks` ("Severity mapping", in `stride-completing-tasks/manual-testing-findings.md` — Critical → `critical`, High and Moderate → `important`, Minor → `minor`, absent/unrecognized → `important`). Only a mapped `critical` that survived the relatedness gate above triggers this; out-of-scope High, Moderate and Minor findings are recorded in the existing carriers, are **never** appended to `issues[]`, and change nothing else (a related finding at those severities was already fixed in-task by the gate). Test each Critical separately when a session returns several; one introduced Critical is enough to escalate.
...
DISPATCH DECOMPOSER WHEN:
  The resolved matrix row says Run in the decomposer column

...
This is background on the server-side rollout for `explorer_result` and `reviewer_result`. **The rules themselves are in SKILL.md** — that both fields are required, where each summary comes from, and what the skip form carries — and are deliberately not repeated here; `stride-completing-tasks` owns their shapes.
...
**Why it matters:** an agent that lags the rollout starts getting `422` rejections on the flip day. What to actually do about it — persisting the Step 3 and Step 5 figures into the Step 7 payload, and the skip form — is stated in SKILL.md, which is where it binds.
